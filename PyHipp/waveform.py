...
class Waveform(DPT.DPObject):
    ...
    def plot(self, i = None, ax = None, getNumEvents = False, getLevels = False,\
        ...
        def sortedDict():
            return list(sorted(self.array_dict.items(), key=lambda item:item[1]))
        ...
        if getNumEvents:  
            if self.current_plot_type == plot_type:
                ...
                return self.numSets, sortedDict()[i-1][1] + 1                
        ...
        if plot_type == 'Channel':
            ...
            cend = advals[i]
            currch = cstart
            # By default switch off subplot titles for array plots.
    ...
    def plot_data(self, i, ax, plotOpts, isCorner):
        ...
        if (not plotOpts['TitleOff']) and isCorner:
            ax.set_title(self.dirs[i])
                    
        if (not plotOpts['LabelsOff']) and isCorner:
